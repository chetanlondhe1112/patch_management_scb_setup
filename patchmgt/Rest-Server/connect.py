...
class Connection:
    ...
    async def DB_Execute(self,query):
        try:
            #async with self.DBconnect:
            log.debug("processing DB_execute function")
            log.debug("processing DB_execute function: {}",query)
            async with self.DBconnect.cursor() as cursor:
                await cursor.execute(query)
                db_out_data = await cursor.fetchall()
                log.debug("DB_execute process after fetcall :{}",db_out_data)
                return db_out_data
            #async with self.DBCur as cursor:
                #await cursor.execute(query)
                #db_out_data = await cursor.fetchall()
                #return db_out_data
        except Exception as e:
            log.debug("error in DB_execute {}",e)
