...
class Processing:
    ...
    async def ProcessMessages(self,msg):
        try:
            EventTypeMap = {
                   '1':"Error",
                   '2':"Warning",
            ...
            for paramsData in msg['paramsUsageData']:
                ...
                if paramValues["paramName"] in self.eventParamNames:
                    ...
                    log.debug("ProcessMessages result {}",result)
                    if result != None:
        ...
        try:
            ...
            for paramsData in msg['paramsUsageData']:
                ...
                if paramValues["paramName"] in self.eventParamNames:
                    ...
                    await self.ProcessMessages(msg)
        ...
        try:
            ...
            if ip in self.monDevMetricsParams:
                ...
                try:
                    ...
                    res = await self.Check_WmiEventMonitor()
                    currentTime = datetime.now()
                    params= []
                    if len(res)>0:
                        for i in res:
                            log.debug("Sending param to getWQL -> {}".format(i[1]))
                            query = await self.getWQL()
                            ip = i[2]
                            param = i[6]
                            eventId = i[5]
                            periodicity = i[4]
                            ...
                            if (lastFetchedTime - currentTime).total_seconds() > 300:
                                await self.connection.DB_Execute(updateQuery)
                            query = query + " WHERE SourceName = '{0}' AND TimeGenerated >= '{1}' AND TimeGenerated <= '{2}'".format(param,(currentTime-timedelta(minutes=5)).strftime("%Y%m%d%H%M%S.%f%z")+'-000',currentTime.strftime("%Y%m%d%H%M%S.%f%z")+'-000')
    ...
    async def Check_WmiEventMonitor(self):
        try:
            query = "select * from eventmonitoringrules"
    ...
    async def Delete_WmiEventMonitor(self,ip,paramname):
        try:
