...
class Processing:
    ...
    async def ProcessMessages(self,msg,conn):
        try:
            ...
            for i in msg['paramsUsageData']:
                pv = dict(i)
                log.debug("@VS ip -> {} | param -> {} | {} and paramsData = {}".format(msg['ip'], pv['paramName'], pv, msg['paramsUsageData']))
    ...
    async def postProcess(self, msg):
        try:
            ...
            agentdevice_Count=await self.connection.DB_Execute(check_query)
            count=len(agentdevice_Count)
            if count == 0:
                values=[(ipAddress,'WMIAGENT','1')]
                updquery="INSERT INTO your_table (deviceip,label,counter) VALUES(%s,%s,%s)"
                await self.connection.DB_Insert(updquery,values)
            ...
            for paramsData in msg['paramsUsageData']:
                ParamDct=dict()
                paramValues = dict(paramsData)
                if paramValues["paramName"] in self.eventParamNames and str(paramValues['Error']) == '0':
